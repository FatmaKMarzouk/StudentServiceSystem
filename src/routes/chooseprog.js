          ...
          connection.query(
            ...
            function (error, results2, fields) {
              if (results2.length > 0) {
                ...
                console.log("bada2t teba3a results2");
                response.status(200).send({ error: false, results2 });
      ...
      function (error, results0, fields) {
        if (results0.length > 0) {
          ...
          if (flag == 1) {
            ...
            connection.query("USE AlexUni");
            connection.query(
              "INSERT INTO Requests (StudentID,ServiceName,ServiceNameAr,Data,Amount,Paid,DatePaid,done,received,FacultyName) VALUES( ?,?,?,?,?,?,?,?,?,? ) ",
              [
                username,
                "Choose Program",
                "اختيار البرنامج",
